...
void WS28xx_DisplayTask(void)
{
	...
	if ((HAL_GetTick() - tickstart) > 100)
	{
		...
		switch(dis_cmd)
		{
			...
			break;
			case DIS_CMD_LORA_RX_DONW:
				WS28xx.SetPixelColor_RGB(0, 0xFF, 0xFF, 0, LIGHT_DUTY);
			break;
			case DIS_CMD_LORA_TX_DONW:
				WS28xx.SetPixelColor_RGB(0, 0x8A, 0x2B, 0xE2, LIGHT_DUTY); 
			break;	
