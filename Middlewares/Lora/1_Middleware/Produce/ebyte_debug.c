    ...
    if( length == EBYTE_CMD_PACKAGE_LENGTH )
    {
        ...
        if( *p++== EBYTE_CMD_PACKAGE_START &&  *p++== EBYTE_CMD_PACKAGE_START &&  *p++== EBYTE_CMD_PACKAGE_START)
        {
            ...
            case EBYTE_CMD_TEST_WIRELESS:
                  ...
                  *tLength    = EBYTE_CMD_PACKAGE_LENGTH;
              break;
            case EBYTE_CMD_TEST_PARAM:
//                  *p++;//指向下一byte数据
//                  PC_Frequency |=   ((uint32_t)(*p++) << 24);
//                  PC_Frequency |=   ((uint32_t)(*p++) << 16);
//                  PC_Frequency |=   ((uint32_t)(*p++) << 8);
//                  PC_Frequency |=   *p++ ;
//                  PC_Power      =   *p;
//                  E22x_SetInit(PC_Frequency,PC_Power);
              break;
            case EBYTE_CMD_TEST_SEND:
//                  uint8_t txByte = 0x80;
//                  E22x_GoTransmit(&txByte,1);
              break;
            }
