...
#include "ebyte_e32x.h"
#include "board.h"

/// 识别码：模块类别
#if defined(EBYTE_E32_400M20S)
static uint8e_t E32x_NameString[] = "E32-400M20S";
#elif defined(EBYTE_E32_900M20S)
static uint8e_t E32x_NameString[] = "E32-900M20S";
#endif

/// 识别码：驱动程序版本号
#define EBYTE_E32_PROGRAM_TYPE 0x10
...
/// 状态标识
static E32x_Status_t E32x_Status = GO_INIT;

/// 接收数据缓存
static uint8e_t E32x_ReceiveBuffer[0xff] ;
 ...
 */
static void E32X_SetAntenna( uint8e_t mode )
{
 ...
 * @param size    寄存器数量
 */
static void E32x_SetRegisters( uint8e_t address, uint8e_t* data, uint8e_t size )
{
    uint8e_t i;
    /* CS片选 低电平选中 */
 ...
 * @param size    寄存器数量
 */
static void E32x_GetRegisters( uint8e_t address, uint8e_t* data, uint8e_t size )
{
    uint8e_t i;
    /* CS片选 低电平选中 */
    Ebyte_Port_SpiCsIoControl( 0 );
 ...
 * @param data    写入数据
 */
static void E32x_SetRegister( uint8e_t address, uint8e_t data )
{
    E32x_SetRegisters( address, &data, 1 );
}
 ...
 * @param data    写入数据
 */
static uint8e_t E32x_GetRegister( uint8e_t address )
{
    uint8e_t data;
    E32x_GetRegisters( address, &data, 1 );
    return data;
}
 ...
 */
static void E32x_SetFIFO( uint8e_t* data, uint8e_t size )
{
    E32x_SetRegisters( 0, data, size );
}
 ...
 */
static void E32x_GetFIFO( uint8e_t* data, uint8e_t size )
{
    E32x_GetRegisters( 0, data, size );
}
 ...
 */
void E32x_SetGPIO( uint8e_t regDioMapping1, uint8e_t regDioMapping2 )
{
    E32x_SetRegister( 0x40, regDioMapping1 );
    E32x_SetRegister( 0x41, regDioMapping2 );
}
 ...
 *      @arg IRQ_CADDETECTED
 */
void E32x_SetIRQ( uint8e_t irq )
{
    uint8e_t reg_value;
    /* 开启中断的机制为对应为置 0 */
    reg_value = ~irq;
    E32x_SetRegister( 0x11, reg_value );
}
 ...
 */
uint8e_t E32x_GetIRQ( void )
{
    /* 读取中断状态 寄存器地址:0x12 */
    return E32x_GetRegister( 0x12 );
}

/*!
 * @brief 清除所有中断标识
 */
void E32x_ClearIRQ( void )
{
    uint8e_t reg_value = 0;
    /* 读取中断状态 寄存器地址:0x12 */
    reg_value = E32x_GetRegister( 0x12 );
    /* 中断清除机制为回写对应位 1 */
    E32x_SetRegister( 0x12, reg_value );
}

/*!
 * @brief 进入休眠模式
 */
void E32x_SetSleep( void )
{
    uint8e_t result;
    /* 读取模式控制寄存器 寄存器地址:0x01   */
    result = E32x_GetRegister( 0x01 );
    /* bit2至bit0 3位表示模式 0代表SLEEP模式  */
    result &= 0xF8;//bit清零
    /* 回写 */
    E32x_SetRegister( 0x01, result );
    /* 状态记录 */
    E32x_Status = GO_SLEEP;
}

/*!
 * @brief 进入待机配置模式
 */
void E32x_SetStby( void )
{
    uint8e_t result;
    /* 读取模式控制寄存器 寄存器地址:0x01   */
    result = E32x_GetRegister( 0x01 );
...
void E32x_SetTransmit( void )
{
    ...
    E32x_Status = GO_WAIT_TRANSMIT;
}

/*!
 * @brief 进入接收模式
 */
void E32x_SetReceive( void )
{
    uint8e_t result;
    /* 读取模式控制寄存器 寄存器地址:0x01   */
    result = E32x_GetRegister( 0x01 );
 ...
 *
 * @note 预定义参数见ebyte_E32x.h中的宏定义 例如 E32X_LORA_BANDWIDTH
 */
uint8e_t E32x_SetLoRaInit( uint32e_t frequency, uint8e_t bandwidth, uint8e_t spreading_factor, uint8e_t coding_rate,
                           uint8e_t crc, uint16e_t preamble_length, uint8e_t sync_word, int8e_t output_power, uint8e_t low_datarate )
{
    ...
    E32x_SetRegisters( 0x06, config, 3 );  //寄存器起始地址:0x06  共计3字节
    /* 设置扩频因子 SF
    ...
    {
        reg_value = E32x_GetRegister( 0x31 );   //配置DetectionOptimize 寄存器地址:0x31 bit2至bit0
        reg_value = ( reg_value & 0xF8 ) | 0x05;//0x05专用于SF6
        E32x_SetRegister( 0x31, reg_value );
    }
    else
    {
        reg_value = E32x_GetRegister( 0x31 );   //配置DetectionOptimize 寄存器地址:0x31 bit2至bit0
    ...
    reg_value = E32x_GetRegister( 0x1E );       //配置SpreadingFactor 寄存器地址:0x1E bit7至bit4
    reg_value = ( reg_value & 0x0F ) | ( spreading_factor << 4 );
    E32x_SetRegister( 0x1E, reg_value );
    /* 设置编码率 CR */
    if( coding_rate < 1 || coding_rate > 4 )
    {
        return 1;    //参数检查
    }
    reg_value = E32x_GetRegister( 0x1D );       //配置CodingRate 寄存器地址:0x1D bit3至bit1
    reg_value = ( reg_value & 0xF1 ) | ( coding_rate << 1 );
    E32x_SetRegister( 0x1D, reg_value );
    /* 设置数据包校验 CRC */
    if( crc > 1 )
    {
        return 1;    //参数检查
    }
    reg_value = E32x_GetRegister( 0x1E );       //配置RxPayloadCrcOn 寄存器地址:0x1E bit7至bit4
    ...
    reg_value = E32x_GetRegister( 0x1D );       //配置Bw  寄存器地址:0x1D bit7至bit4
    reg_value = ( reg_value & 0x0F ) | ( bandwidth << 4 );
    E32x_SetRegister( 0x1D, reg_value );
    /* 设置前导码长度 */
    config[0] = ( preamble_length >> 8 ) & 0x00FF;
    config[1] = preamble_length & 0xFF;
    E32x_SetRegisters( 0x20, config, 2 ); //寄存器起始地址:0x20  共计2字节
    /* 设置同步字 */
    E32x_SetRegister( 0x39, sync_word ); //寄存器起始地址:0x39
    /* 设置低空速优化 */
    if( low_datarate > 1 )
    {
        return 1;    //参数检查
    }
    reg_value = E32x_GetRegister( 0x26 );       //配置LowDataRateOptimize 寄存器地址:0x26 bit3
    reg_value = ( reg_value & 0xF7 ) | ( low_datarate << 3 );
    E32x_SetRegister( 0x26, reg_value );
    ...
    {
        E32x_SetRegister( 0x4D, 0x87 );          //配置RegPaDac 寄存器地址:0x4D  模块可控输出功率范围切换为 5dBm至20dBm
        reg_value  = E32x_GetRegister( 0x09 );   //配置PaSelect 寄存器地址:0x09
        ...
        E32x_SetRegister( 0x09, reg_value );
        E32x_SetRegister( 0x0B, 0x3B );          //配置RegOcp   寄存器地址:0x0B  扩大电流 过流保护开启
    }
    else                  // 2-17 dBm
    {
        E32x_SetRegister( 0x4D, 0x84 );          //配置RegPaDac 寄存器地址:0x4D  模块可控输出功率范围切换为 2dBm至17dBm
        reg_value  = E32x_GetRegister( 0x09 );   //配置PaSelect 寄存器地址:0x09
 ...
 * @note
 */
uint8e_t E32x_GoReceive( void )
{
    /* 模式切换:待机 */
    E32x_SetStby();
    /* 关闭跳频 寄存器地址:0x24 */
    E32x_SetRegister( 0x24, 0 );
    ...
    E32x_SetRegister( 0x0F, 0 );
    /* IO: 开启接收电路 */
    E32X_SetAntenna( ANTENNA_RXEN );
    /* 允许接收中断 */
    E32x_SetIRQ( IRQ_RXDONE );
    /* 清除中断标识 */
    E32x_ClearIRQ();
    /* 切换为接收模式 */
    E32x_SetReceive();
...
uint8e_t E32x_Init( void )
{
    ...
    /* 初始化：LoRa 或 FSK */
    switch( E32X_MODULATION_TYPE )
    {
        case 0:
            /* 执行LoRa模式初始化 配置见宏定义 */
            E32x_SetLoRaInit( E32X_FREQUENCY_START,        //载波频率
                              E32X_LORA_BANDWIDTH,         //频宽
                              E32X_LORA_SPREADING_FACTOR,  //扩频因子
                              E32X_LORA_CORING_RATE,       //编码率
                              E32X_LORA_CRC,               //CRC开关
                              E32X_LORA_PREAMBLE_LENGTH,   //前导码长度
                              E32X_LORA_SYNC_WORD,         //同步字
                              E32X_OUTPUT_POWER,           //输出功率
                              E32X_LORA_LOW_DATARATE );    //低空速优化
            break;
        case 1:
            //FSK
            break;
 ...
 */
uint8e_t E32x_TaskForPoll( void )
{
    uint8e_t irqStatus;
    /* 获取模块内部中断标识状态 */
    irqStatus = E32x_GetIRQ();
    ...
    if( irqStatus != 0 )
    {
        ...
        if( irqStatus & IRQ_RXDONE )
        {
            ...
            uint8e_t rxAddress = E32x_GetRegister( 0x10 );
            /* 获取最新接收数据的长度  寄存器地址:0x13 */
            uint8e_t rxLength = E32x_GetRegister( 0x13 );
            /* 设置SPI读取FIFO 起始地址  寄存器地址:0x0D */
            E32x_SetRegister( 0x0D, rxAddress );
            /* 读取FIFO数据 */
            E32x_GetFIFO( E32x_ReceiveBuffer, sizeof(E32x_ReceiveBuffer) );
            /* 回调用户函数 */
            Ebyte_Port_ReceiveCallback( 0x0001, E32x_ReceiveBuffer, rxLength );
        }
        /* 清除中断 */
        E32x_ClearIRQ();
		E32x_SetReceive();
 ...
 */
uint8e_t* E32x_GetName( void )
{
    return E32x_NameString;
}
 ...
 * @note 例如0x10 代表V1.0
 */
uint8e_t E32x_GetDriverVersion( void )
{
    return  EBYTE_E32_PROGRAM_TYPE;
}
 ...
 *        内部错误     GO_ERROR         =0x08
 */
uint8e_t E32x_GetStatus( void )
{
    return ( uint8e_t )E32x_Status;;
}
