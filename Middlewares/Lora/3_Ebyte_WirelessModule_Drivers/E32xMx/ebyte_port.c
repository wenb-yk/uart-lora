...
uint8e_t Ebyte_Port_SpiTransmitAndReceivce( uint8e_t send )
{
    uint8e_t result = 0;
    /*!必须提供: SPI接口 */
    HAL_SPI_TransmitReceive(&hspi1, &send, &result, 1, 0xff);
    return result;
}
...
void Ebyte_Port_RstIoControl( uint8e_t cmd )
{
    ...
    {
        /*!必须提供: NRST控制IO 输出高电平 */
        Ebyte_BSP_E32ResetIoHigh();//示例
    }
    else
    {
        /*!必须提供: NRST控制IO 输出低电平 */
        Ebyte_BSP_E32ResetIoLow();//示例
    }
}
...
void Ebyte_Port_TxenIoControl( uint8e_t cmd )
{
    if( cmd == 1 )
    {
        ...
        Ebyte_BSP_E32TxIoEnable();//示例
    }
    else
    {
        /*!可选: 目标硬件电路对应TXEN控制IO 输出低电平 */
        Ebyte_BSP_E32TxIoDisable();//示例
    }
}
...
void Ebyte_Port_RxenIoControl( uint8e_t cmd )
{
    if( cmd == 1 )
    {
        ...
        Ebyte_BSP_E32RxIoEnable();//示例
    }
    else
    {
        /*!必须提供: 目标硬件电路对应RXEN控制IO 输出低电平 */
        Ebyte_BSP_E32RxIoDisable();//示例
    }
}
...
//    result = Ebyte_BSP_E32Dio0Read();//示例
    return result;
}
